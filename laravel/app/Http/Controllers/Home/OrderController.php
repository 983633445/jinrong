...
class OrderController extends CommonController
{
    ...
    public function checkout(){
        $param= $input = Input::all();
        $productId = explode(',',$param['productId']);
        $goodsId = explode(',',$param['goodsId']);
        $goodsNum = explode(',',$param['num']);
        $payNum = array_combine($goodsId,$goodsNum);

        $obj = new ShopCartController();
        $goodsInfo = $obj->cartList($data = array(),$productId,$goodsId,$payNum);

        $redpacket = $this->getPacket();
        //红包
        $redpacket =json_decode($redpacket,true);
//        var_dump($redpacket);die;
        //地址联动信息展示
        ...
        $obj = new \App\Models\address();
        $address= $obj->select();
        return view('Home/checkout',['sheng'=>$sheng,'address'=>$address,'redpacket'=>$redpacket,'goodsInfo'=>$goodsInfo]);
